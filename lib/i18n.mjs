...
// Supported languages
const SUPPORTED_LOCALES = ['en', 'fr', 'hu'];
...
/**
 * Get locale data for a specific language, with fallback.
 * @param {string} lang - Language code
 * @returns {object} Locale data
 */
export function getLocaleForLanguage(lang) {
  const normalized = lang?.toLowerCase()?.slice(0, 2);
  return loadLocale(SUPPORTED_LOCALES.includes(normalized) ? normalized : DEFAULT_LOCALE);
}
 ...
 */
export function getLLMPrompt(lang = getLanguage()) {
